...
Aura Weather is a polished React weather dashboard built for fast scanning, resilient data handling, and a calm day-planning experience. The header states the thesis under the brand: **"Today's conditions, honest about what it doesn't know."**
...
- Compare today's forecast high against the 30-year average high for the date, from the Open-Meteo historical archive, when available.
- Read a severe alert in full: whether it is pending or already in effect, the alerted counties, the recommended action as a single chip, NOAA/NWS's own protective-action text inline, and the complete advisory behind a disclosure — with explicit unsupported-region messaging where coverage stops.
- Track observed precipitation on an interactive radar map (Leaflet + RainViewer) with a scrubable timeline, loaded lazily so it never blocks first paint — with a severe alert's own outline drawn on it, and a plain caption when the alert covers whole counties and has no outline to draw.
...
- Supabase (optional rain-alert backend + saved-city backup)
- Plain CSS
- Playwright + axe-core
- Open-Meteo APIs
...
```text
src/                         # illustrative excerpt: key modules, not exhaustive
  ...
  components/                # React surface
    ...
    radar/                   #   RadarPanel, RadarMap, RadarTimeline
    HeroCard, RainCard, ForecastCard, NowcastCard,
    StormWatch, HourlyCard, AlertsCard, AtmosphereBento,
    CitySearch, WeatherIcon, AppErrorBoundary, PanelErrorBoundary,
    radar/ (RadarPanel, RadarMap, RadarTimeline, RadarLegend)
...
- `npm run lint` passes
- `npm test` passes (`<!--n:unit-tests-->1263<!--/n-->` tests across <!--n:unit-suites-->283<!--/n--> suites, including the <!--n:render-tests-->383<!--/n--> React render tests via `jsdom` + `esbuild`)
...
- Playwright coverage — <!--n:e2e-behavioural-->49<!--/n--> behavioral checks plus <!--n:e2e-capture-->8<!--/n--> screenshot/asset
  ...
  - assistive-tech cue check for the missing-data trust contract (`aria-label="No data available"` spans on the missing stats)
...
- **Climatology is cached per place and date.** The archive's answer for a coordinate and calendar day is a 30-year normal, and it changes only when the year rolls over. It is kept in `localStorage` keyed by rounded coordinates, month-day, year and unit (`climatologyCache.js`), so returning to a city or refreshing its forecast costs no archive request, and the climate line still appears offline for a place seen this year.
...
- **The nowcast speaks at the resolution its data supports.** The series
  behind it is a *chance* of precipitation sampled every quarter hour, so a
  threshold crossing at a slot is not rain beginning at that minute — and the
  quarter-hour cadence is not itself a resolution claim, since the provider
  models those steps natively in some regions and interpolates them from hourly
  data in others. Start and duration are bucketed ("within the next half hour",
  "passing quickly") in one place that both the sentence and the tiles read, and
  the push alert quotes the window it actually scanned rather than the lead-time
  setting, which rounds up to whole slots.
- **Provider timestamps become real instants, through the location's zone.**
  Open-Meteo sends naive local strings (`timezone=auto`), which `new Date()`
  reads in the *device's* zone. The app used to reframe "now" the same way so
  both sides were wrong together — which cancels out on 363 days a year, but
  not on the device zone's own DST days, where two hourly slots collapse onto
  one instant in spring and an hour is unreachable in autumn. `utils/zonedTime.js`
...
- **The display locale has one home.** Every user-facing date and time is
  ...
  source tree and fails on any locale named outside that module, whether
  it was left to the runtime or written out as a literal.
...
placeholders, the missing-data footnote, and the muted `.is-missing`
treatment):
...
3. **Component fallback rendering** — every consumer that displays a
   ...
   path (no more `—°F`), and when any atmosphere reading is missing
   the panel appends a footnote — "— means the provider didn't
   report that reading. It isn't a zero." — so the user is not left
   to guess.
4. **Visual + screen-reader cue** — missing readings render with an
   `.is-missing` modifier (muted color, normal weight) plus an
   `aria-label="No data available"` span, so assistive tech announces
   "no data available" instead of speaking the literal "em dash"
   character. The treatment is shared across the hero, forecast,
   rain, and storm cards.
...
- **React render (`HeroCard.render.test.mjs`, <!--n:herocard-render-tests-->25<!--/n--> tests)** — the
  ...
  drop when the headline reading is missing.
...
("Portfolio demo: showing the missing-data trust contract. Live
providers are not queried.") and renders `src/mocks/missingData.js`
instead of querying live providers — a Playwright check watches the
network and asserts the route makes zero provider requests. Missing
readings render honest "—" placeholders with `aria-label="No data
available"` cues, the atmosphere panel appends the "— means the
provider didn't report that reading. It isn't a zero." footnote, and
the radar slot holds a card reading "Radar not queried in this demo"
instead of a mounted map, so no tile host is ever contacted. The
current temperature stays plausible so the dashboard still looks
...
- **The map came back** - the radar's basemap was covered in "API KEY REQUIRED" watermarks on the live site: CARTO had begun gating its raster basemaps behind a key, and the failure was invisible to every check the app had, because a watermarked tile is still HTTP 200 with a valid PNG body — the watermark *is* the image. Both CARTO variants are gated, so swapping light for dark was not a fix. The basemap is now OpenStreetMap's standard raster, chosen because its terms were read rather than assumed: the OSMF tile policy welcomes this use, requires visible attribution that is not hidden behind a toggle, and forbids prefetching — all three of which the app satisfies. Its host, attribution and zoom ceiling live in `src/domain/basemap.js`, the Content-Security-Policy names that host exactly so a stale build cannot silently render the watermark again, and the road map is desaturated in CSS so the precipitation echoes keep the colour their legend depends on.
- **Where the alert is** - a severe alert's polygon is drawn on the radar map, beneath the location marker, so "does this cover me" is answered by looking rather than by reading county names. NWS sends an outline on storm-based products and nothing at all on zone-issued ones (2 of the 5 recorded features carry one), so an alert with no outline gets a plain caption — "County-level alert — not drawn on map", counted once there is more than one — instead of silently having nothing on the map. The GeoJSON `[lon, lat]` to Leaflet `[lat, lon]` swap lives in one pure module with its order pinned by tests: passing the pair through unswapped does not throw, it just draws the shape at a latitude that does not exist. A `MultiPolygon`, a polygon with a hole, or a ring carrying one unusable vertex is not drawn at all rather than drawn in part.
- **Pending or happening** - a severe alert now says where it sits in its own window: "Starts within 12 hours", "In effect now", "Ends within the hour", beside the absolute "Until …". Driven by CAP `onset` (the hazard start) rather than `effective` (the message issue time) — they differed on 169 of 243 live alerts — and bucketed coarsely, with no remaining-time numeral, because a long-fuse onset is a revised forecast rather than a countdown. An onset after its own hazard end is reported as malformed and renders no phrase; an alert with no onset falls back to its issue time, and one with neither renders nothing rather than a placeholder.
- **Actionable severe alerts** - the alert card used to render an event name, one headline line, a priority badge and an expiry, and discard everything else NWS sent. It now shows the alerted area (`areaDesc`), the recommended action from CAP `response` as a chip ("Take shelter", "Avoid the area"), the `instruction` prose inline, and `description` behind a "Full advisory text" disclosure. Drills are filtered: a transmission whose CAP `status` is not `Actual` no longer renders as an active hazard. Absent fields stay silent rather than announcing their absence — `instruction` is missing on ~40% of active alerts but on none carrying `urgency: Immediate`, so an "unavailable" line would fire almost only on marine advisories.
...
- **Honest Lighthouse budgets** - the CI gate enforces performance 0.85, accessibility 0.95, best-practices 0.9, and SEO 0.9 (up from a 0.5 performance floor). The audited route — the deterministic `?mock=missing` app shell, desktop config — measured 99/100/100/100 on 2026-08-30.
...
- **Missing-data footnote** — when any atmosphere reading is missing the panel appends "— means the provider didn't report that reading. It isn't a zero." so the user understands *why* a value is shown as `—`. (An earlier hero-card helper note was retired when the stat grid moved into the Atmosphere bento.)
...
- **CSS co-location** — `App.css` shrank from 2,067 lines (it sits at <!--n:app-css-lines-->1211<!--/n--> today) as `DataTrustMeta`, `InfoDrawer`, `AppShell`, `StatusStack`, the bento dashboard layout, and the entire header surface moved next to their owning components.
...
- **Lighthouse budget passes locally** against the deterministic `?mock=missing` app shell, but real-world performance varies with live provider latency. The CSS and JS footprint shrunk substantially during the audit (App.css 2,067 → <!--n:app-css-lines-->1211<!--/n--> lines), but image pre-caching and paint-cost tuning would still be useful next wins.
...
- **Trust-contract shot** — load `/?mock=missing` to reproduce the labelled missing-data demo state instantly. The dashboard renders muted `—` placeholders (announced as "No data available" to assistive tech) for humidity, pressure, dew point, daily high/low gaps, and precipitation gaps, with the atmosphere footnote explaining "— means the provider didn't report that reading. It isn't a zero." and a radar card reading "Radar not queried in this demo" because the route provably makes zero provider requests. This is the strongest single signal of the trust narrative.

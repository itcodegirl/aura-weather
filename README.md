...
- `npm run lint` passes
- `npm test` passes (`<!--n:unit-tests-->1215<!--/n-->` tests across <!--n:unit-suites-->270<!--/n--> suites, including the <!--n:render-tests-->371<!--/n--> React render tests via `jsdom` + `esbuild`)
...
- Playwright coverage — <!--n:e2e-behavioural-->48<!--/n--> behavioral checks plus <!--n:e2e-capture-->8<!--/n--> screenshot/asset
...
- **CSS co-location** — `App.css` shrank from 2,067 lines (it sits at <!--n:app-css-lines-->1152<!--/n--> today) as `DataTrustMeta`, `InfoDrawer`, `AppShell`, `StatusStack`, the bento dashboard layout, and the entire header surface moved next to their owning components.
...
- **Lighthouse budget passes locally** against the deterministic `?mock=missing` app shell, but real-world performance varies with live provider latency. The CSS and JS footprint shrunk substantially during the audit (App.css 2,067 → <!--n:app-css-lines-->1152<!--/n--> lines), but image pre-caching and paint-cost tuning would still be useful next wins.

...
- Read a severe alert in full: the alerted counties, the recommended action as a single chip, NOAA/NWS's own protective-action text inline, and the complete advisory behind a disclosure — with explicit unsupported-region messaging where coverage stops.
...
- `npm run lint` passes
- `npm test` passes (`<!--n:unit-tests-->1029<!--/n-->` tests across <!--n:unit-suites-->242<!--/n--> suites, including the <!--n:render-tests-->343<!--/n--> React render tests via `jsdom` + `esbuild`)

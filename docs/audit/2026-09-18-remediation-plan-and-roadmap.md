...
**Revised:** 2026-09-19 — reconciliation pass against `main` @ `fb4f747`. Documentation only, no source changes. Corrected: AUD-004's milestone row (read "In review"; PR #237 had merged), AUD-003's Definition-of-Done item 10, AUD-019's No Action status, and the absence of any record of the 2026-09-18 radar basemap outage. Sections touched: §2C, §3, §8, §15, §16.
...
Claims added by the **2026-09-19 reconciliation pass** carry two further labels: `[ran: cmd]` — a command run against this repo at `fb4f747` on 2026-09-19 — and `[api: call]` — GitHub state read through the API on that date. Both are point-in-time. A pull-request state labelled `[api:]` was true when it was read and says nothing about any later moment; re-read it before relying on it.
...
### C. No action — 2 findings, plus one reconsidered
...
**Acceptance criteria.** All met on `feat/alerts-geometry` (2026-09-18).
1. ✅ A Polygon-carrying alert draws at the correct location — e2e asserts a non-empty `d` on `path.radar-alert-shape`, because a clipped shape is an empty path that still reports visible.
2. ✅ A geometry-less alert draws nothing and produces the caption.
3. ✅ Malformed geometry never throws and never draws a partial shape — one bad vertex voids the ring.
4. ✅ The location marker is never occluded — the outline renders in its own pane at z-index 350; e2e compares the computed z-index of both panes.
5. ✅ `?mock=missing` still makes zero provider requests — the radar map is never mounted on that route, and the existing e2e guard is unchanged.
...
| 4 — AUD-003 onset timing | **Shipped** | PR #236, `feat/alerts-onset-timing`, merged 2026-09-18. `onsetAt` normalised from CAP `onset` (additive; `startsAt` untouched). `src/domain/alertTiming.js` — `describeAlertTiming(onsetAt, endsAt, expiresAt, nowMs)` → `{ phase, phrase }`, phases `pending` / `active` / `ended` / `invalid` / `unknown`. The malformed case is onset-after-**ends**, checked before any phase; `expires` before `onset` is normal (99 of 243 live alerts) and never takes part in the comparison, which reads `ends` alone — an alert with no `ends` (20 of 243) and a known future onset is `pending`, not malformed. Lead buckets are coarse and carry no remaining-time numeral — "Starts within the hour" / "in the next few hours" / "within 12 hours" / "within a day" / "within 2 days" / "in more than 2 days" — matching the nowcast register; "In effect now" and "Ends within the hour" for active. Rendered as plain text beside the existing "Until …", no live region. The card resolves `onsetAt ?? startsAt` before calling, so the fallback to `effective` (1 of 243) is the caller's, and an alert restored from a snapshot written before `onsetAt` existed (key absent, not null) renders no phrase rather than reading `effective` as the hazard start. Two defects found by adversarial review of the first four commits and fixed in the same PR: the malformed check compared onset against the RESOLVED end, so an alert with no `ends` (20 of 243) and an earlier `expires` was wrongly reported invalid; and the restored-cache case above. Case-study screenshot (DoD item 10): **the deferred capture has been taken.** `docs/redesign/instrument/captures/01-alerts.jpg` photographs the production deploy carrying two live NWS alerts, and shows five merged tickets working at once — `areaDesc` (AUD-008), the response chips (AUD-009), the inline instruction and the full-advisory disclosure (AUD-002 / AUD-021), the onset phrases this ticket added (AUD-003) and the hazard-end "Until" line (AUD-023) — on AUD-005's shared severity ladder. The file is **not on `main`**: it exists only on `chore/instrument-captures` `[ran: git ls-tree -r --name-only origin/chore/instrument-captures]`, which was an open draft as PR #240 on 2026-09-19 `[api: list_pull_requests]`. **Item 10 is therefore satisfiable but not yet satisfied — it closes when #240 merges,** and this row is the one place that needs changing then. |
| 5 — AUD-005 rung labels | **Resolved** | Signed off 2026-09-17, recorded in the milestone spec §4 "Signed-off labels". |
| 6 — AUD-004 / WP-2 alert geometry | **Shipped** | PR #237, `feat/alerts-geometry`, merged 2026-09-18 `[ran: git log --format='%h %ad %s' --date=short]`. This row read "In review" until the 2026-09-19 reconciliation; the rest of the cell is unchanged and still describes what shipped. First ticket of Phase 2's remainder, and the first to touch the radar. `normalizeAlert` now reads `feature.geometry` — the only field on the alert model from outside `properties` — validated to a drawable single-ring `Polygon` or null and kept in the provider's GeoJSON order, because this model is also what persists to the snapshot cache. `src/domain/alertGeometry.js` holds the swap: `toLeafletPositions` converts `[lon, lat]` to `[lat, lon]`, `summariseAlertGeometry` splits an alert list into shapes and an undrawable count, `undrawnAlertsNote` writes the caption. `MultiPolygon`, a polygon with a hole, and any ring carrying one unusable vertex are rejected whole — never a partial shape. The outline renders in a named Leaflet pane at z-index 350, between the radar tiles (200) and the location marker (400), because Leaflet stacks vector layers by insertion order within a pane and react-leaflet adds each in a passive effect, so JSX order alone cannot satisfy "the marker is never occluded". The panel filters alerts through the same `isAlertActive` the card uses, on the same minute ticker, so a shape on the map always has a row beside it. An alert with no `geometry` key at all — restored from a pre-WP-2 snapshot — is neither drawn nor counted as county-level; both would be claims about a shape nothing recorded. Deferred as specified: `affectedZones` → zone geometry, `MultiPolygon`, polygon interaction, timeline animation. |
| 7 — Radar basemap outage (unplanned, added 2026-09-19) | **Shipped** | PR #238, `fix/radar-basemap`, merged 2026-09-18 `[ran: git log --format='%h %ad %s' --date=short]`. Not an audit finding and not in the register — a production outage found by eye while capturing the live app, recorded here because no register row would otherwise hold it and the plan is where prioritisation reads from. Every basemap tile on the deployed radar rendered "API KEY REQUIRED · carto.com/basemaps/apikey": CARTO had begun gating its Positron raster basemap behind a key. **Why nothing caught it:** the gated tile still returns HTTP 200 with a valid PNG body — the watermark *is* the image — so no status-code probe and nothing in CI can see this class of failure, and the replacement candidates were therefore verified by **rendering** them side by side rather than by fetching them `[read: src/domain/basemap.js:6-13]`. Both CARTO variants are gated, so swapping `light_all` for `dark_all` was not a fix; that was checked by rendering both. The basemap is now OpenStreetMap's standard raster, moved into a new `src/domain/basemap.js` holding `BASE_TILE_URL`, `BASE_ATTRIBUTION` and `BASE_MAX_ZOOM`. OSM was chosen because its terms were read rather than assumed — the OSMF policy requires visible attribution not hidden behind a toggle and forbids prefetching, both of which Aura satisfies — and Esri's Gray Canvas was declined because its terms could not be retrieved, which is the same standard of evidence the data-trust contract applies to weather data `[read: src/domain/basemap.js:17-31]`. CSP `img-src` now names the host exactly so a stale build cannot silently render the watermark again, and the desaturation filter was scoped to the basemap's own layer because `grayscale()` is destructive and was draining RainViewer's echoes of the colour their legend depends on. Tests guard the host, not the pixels. Also recorded in `README.md` `[read: README.md:406]`. **Carried forward as an open risk:** nothing automated checks that the basemap renders map content rather than a message, so a third-party tile host can still fail this way invisibly. |

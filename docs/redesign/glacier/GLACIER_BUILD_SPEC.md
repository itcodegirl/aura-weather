...
1. **Header** — brand + location pill (with search) + °F/°C toggle + settings
2. **Severe-alert banner** (amber, collapses to nothing when none)
3. Group label **"Current conditions"** → **Animated reactive hero**
4. **Hourly forecast** — flowing-curve card (directly under the hero)
5. Group label **"Near-term outlook"** → **Rain outlook + Nowcast** (two cards)
6. Group label **"Risk signals"** → **Storm watch** (restyled, compact)
7. Group label **"Atmosphere"** → **Bento gauges** *(in scope — build)*
8. Group label **"Week ahead"** → **7-day forecast**
9. **Data-trust footer** — timezone · coords · sources (Open-Meteo + NOAA/NWS) · updated time
...
- **Ref:** `aura_animated_reactive_hero` (canonical) + `aura_hero_plus_hourly_curve`.
...
- **Accept:** ☐ sky animates (breathing + sun-glow pulse + 2 parallax cloud layers) · ☐ chips are data-derived AND drop out individually when data missing · ☐ 84px Inter-200 temp · ☐ reduced-motion kills all animation · ☐ side-by-side matches mockup.

### 3.4 Hourly forecast — flowing curve + metric selector  ⭐ (a #81 miss — build in full; refined per `aura-hourly-card-v2`)
- **Ref:** `aura_flowing_hourly_metric_selector` (canonical, **v2**).
- **Layout (v2 — supersedes the original "curve through icons"):** a fixed **condition-icon row** sits directly under the time row; below it, a smooth probability **curve weaves through the per-hour value points**, with a gradient fill + soft glow beneath.
- **Condition icons:** one per hour, **inline SVG (no icon font)** — sun, sun+cloud, cloud, cloud+rain, moon, moon+cloud — **day/night aware** (sun in daylight hours, moon after sunset). Semantic tint: amber sun, glacier-blue rain. (The original icon-font approach rendered nothing when the font wasn't loaded — do not reintroduce it.)
- **Values never collide with the line:** each value rides the curve inside a small **frosted chip with a dark translucent background**; the stroke is drawn *behind* the chips so it is occluded wherever a number sits. The line must not cross any digit (verify on the **Temperature** metric, where values bunch).
- **Fill:** vertical gradient — metric color ~42% just under the curve fading to **0 at the bottom** (no flat slab, no hard bottom edge) — plus a soft doubled glow on the stroke; curve + fill run **full-bleed** to the card edges. `vector-effect:non-scaling-stroke` keeps the stroke crisp under the stretched viewBox.
- **Metric selector** ("Precipitation / Temperature / Wind") re-plots the curve and glides the chips; **icons stay fixed** (condition is independent of metric). Placed **directly under the hero.**
- **Accept:** ☐ inline-SVG condition icons on a fixed row, day/night aware · ☐ curve weaves through value points · ☐ value chips occlude the line (no digit ever crossed — checked on Temperature) · ☐ gradient fill fades to 0 + glow, full-bleed · ☐ selector re-plots curve, icons unchanged · ☐ all text ≥ 11px / AA · ☐ matches `aura-hourly-card-v2`.
...
### 3.6 Storm watch (restyle existing)
- **Ref:** assembled ("Risk signals" group) + `aura_glacier_reactive_hero_alerts_storm`.
- Keep the panel (storm risk + CAPE · pressure trend + sparkline · wind compass + gusts · comfort + dewpoint). **Restyle into the Glacier theme and make it compact.** Warm amber on storm-risk/pressure/comfort cues.
- **Accept:** ☐ all four sub-readings present · ☐ compact Glacier styling · ☐ amber cues retained · ☐ matches mockup.

### 3.7 Atmosphere bento gauges  — ✅ DECIDED: BUILD
- **Ref:** `aura_bento_metric_grid_glacier`.
- **Decision:** Jenna ruled **build** (PR #82 had skipped it). It is **in scope.** The one constraint from that earlier debate stands: **do not duplicate** readings that already live elsewhere as their primary home — the bento is the *at-a-glance instrument panel*, distinct from Storm watch's risk framing. Match the mockup's metric set.
- **Build:** true bento with **mixed spans** (not a flat 3×3): row of 4 small 1×1 gauges (Humidity, UV, Air quality, Pressure) → 2-wide Wind (compass+gusts) + 2-wide Sun arc → 2-wide Dew-point comfort gradient bar + 1×1 Visibility + 1×1 Moon. Standalone tiles at .15 surface. One shared "instrument kit" (thin 7px rounded gauge arcs, one compass style, one sun arc, one bar style; icon+caps label, thin value, qualifier word). Semantic color per metric. **AQI tile intentionally shown in the missing-data state** (dashed border, hollow gauge, muted "—", "Unavailable", "Not reported here") — the trust contract made visible; Jenna loved this as a portfolio detail.
- **Accept:** ☐ mixed spans (not 3×3) · ☐ shared instrument kit · ☐ AQI shown in missing-data state · ☐ no duplication of a metric's primary home (Storm watch / Exposure) · ☐ all text ≥ 11px / AA · ☐ matches mockup.
...
1. **Bento gauges:** **BUILD.** The Atmosphere bento (§3.7) is in scope. Constraint: don't duplicate a metric's primary home.
2. **Accent:** **`#6fb7f2`.** Locked everywhere; change the app's `#8bd3ff` → `#6fb7f2` (§1).

Echo both at the top of the implementing PR so the choices are on the record.
...
- ☐ Header · ☐ Alert banner (collapses when empty) · ☐ **Animated reactive hero (3 layers + data chips)** · ☐ **Flowing-curve hourly + metric selector** · ☐ Near-term outlook (2 cards, spacing fixed) · ☐ Storm watch (restyled compact) · ☐ **Atmosphere bento (mixed spans, AQI missing-state)** · ☐ **7-day: Details pill + drawer + per-day mini hourly** · ☐ Trust footer
- evidence PNGs: ____
...
**Sign-off**
- ☐ Resolved decisions echoed at top of PR: bento = **build**, accent = **`#6fb7f2`** (§5)
